## Analysis of the Accident in the US between 2016 and 2021
...
## states polygons
#us_sf <- data.frame(us_states)
#us_sf

## states names // https://www.ssa.gov/international/coc-docs/states.html
# states_name <- read.csv("data/states_names.csv", sep=";")
...
ggplot(car_accidents, aes(as.factor(month))) + geom_bar() + 
  ...
  #geom_vline(xintercept = mean(car_accidents$month), colour = "green") + 
...
## Weather impact
# How many accidents related to Temperature of that area?
# Check how Humidity is affecting the number of accidents?
...
### step1: Define neighboring polygons (contiguous polygons that share at least one vertex)
...
#la_weights <- nb2listw(nb, style="W", zero.policy=TRUE)
...
### Step 3: (optional): Compute the (weighted) neighbor mean LE values (spatially lagged values)
...
I
# no relationship between Le_avg and Le_avg_lag, the slope will be close to flat (resulting in a Moran’s I value near 0)

# Step 5: Performing a hypothesis test
# hyp0: the LE values are randomly distributed across LA following a completely random process
stat = moran.test(sf_car_accidents2$`nb accidents`, la_weights, alternative="greater")
#stat = moran.test(st_drop_geometry(sf_car_accidents2[,"nb accidents"])[,1], la_weights, alternative="greater")
